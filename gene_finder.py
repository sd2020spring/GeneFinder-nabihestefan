...
def longest_ORF(dna):
    ...
    all = find_all_ORFs_both_strands(dna)
...
def gene_finder(dna):
    ...
    arr = find_all_ORFs_both_strands(dna)

    filtered = list(filter(lambda x: len(x) >= threshhold, arr))

    mapped = list(map(coding_strand_to_AA, filtered))
